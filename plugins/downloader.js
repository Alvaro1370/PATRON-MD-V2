...
smd(
    ...
    try {
      ...
      if (!repoUrl) {
        return await _0x1ae8f8.reply(
          "*Provide Repo URL, e.g., .git https://github.com/STAR-KING0/PATRON-MD_*"
        );
      }
 ...
 smd({
   ...
   category: "downloader",
   filename: __filename,
   use: "<Hii,this is PATRON-MD>"
 }, async (_0x55aba2, _0x56da6b) => {
   try {
     let _0x204f81 = _0x55aba2.reply_text ? _0x55aba2.reply_text : _0x56da6b;
     if (!_0x204f81) {
       return _0x55aba2.reply("*_Example : .tts Hi,I am PATRON-MD whatsapp bot._*");
     }
     try {
 ...
 smd({
   pattern: "sound",
    alias: ["PATRON-MD", "aine","mentalism","alive","waso"],
   ...
   try {
     ...
     let _0x4737bb = {
       audio: _0x2ba501,
       fileName: "PATRON-MD tiktok Sound" + _0x19c223 + ".m4a",
       mimetype: "audio/mpeg",
       ptt: true,
 ...
 smd({
   ...
   try {
     ...
     try {
       ...
       let _0x4b47c3 = "./temp/PATRON-MD " + _0x1d542b[1] + ".mp3";
     ...
     let _0x300d1a = {
       ...
       mimetype: "audio/mpeg",
       fileName: "PATRON-MD--" + _0x1d542b[1] + ".mp3",
       caption: Config.caption,
...
smd(
  ...
  async (m) => {
    try {
      ...
      const message = `╔════P═A═T═R═O═N═══════🍀\n` +
                      `║ *🛐Content:* ${verse.trim()}\n` +  // trim() removes any unnecessary whitespace
                      `║ *✨Reference:* ${reference}\n` +
                      `║ *✨powered By PATRON-MD\n` +
                      `╚════P═A═T═R═O═N════🍀`;
